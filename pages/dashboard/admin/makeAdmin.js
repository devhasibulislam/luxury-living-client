...
const MakeAdmin = () => {
  ...
  const { data: users } = useSWR("https://luxury-living-ll-server.onrender.com/user", fetcher, {
  ...
  const handleAdmin = async (event) => {
    ...
    const { data } = await axios.put(`https://luxury-living-ll-server.onrender.com/user/${email}`, {
      email: email,
    });
